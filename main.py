...
class Student:
    def __init__(self, first_name,last_name, ID, username, password, courses):
...
class Course:
    def __init__(self, name, ID, credit, lecturer, assignment):
        self.name = name
        self.ID = ID
        self.credit = credit
...
def CheckPassword(st_id, ID, password, student_grades):
    ...
    try:
        if student_grades[st_id]:
            if p == student_grades[st_id]['password']:
...
def main():
    ...
    p = CheckPassword(ID, st_id, password, student_grades)
